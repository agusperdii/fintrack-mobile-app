    ...
    return Scaffold(
      ...
      body: ListenableBuilder(
        ...
        builder: (context, _) {
          ...
          // Safety check for DropdownButton: 
          // If the selected month is not in the list (e.g. after logout or data change), 
          // we reset it or temporarily add it to avoid assertion error.
          String? activeMonth = _selectedMonth;
          if (!controller.isLoading && allTxs.isNotEmpty && activeMonth != null && !months.contains(activeMonth)) {
            // Reset to null if not found in current data
            activeMonth = null;
            // We use WidgetsBinding to update the state safely if it changed
